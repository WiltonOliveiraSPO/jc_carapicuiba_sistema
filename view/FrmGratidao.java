...
public class FrmGratidao extends JFrame {

    private JTextField txtCodigo, txtValor, txtData;
    private JComboBox<Membro> cbMembro;
    private JComboBox<String> cbTipo;

    private List<Gratidao> lista;
    ...
    // ================= TIPOS FIXOS =================
    private final String[] TIPOS_GRATIDAO = {
        "Mensal",
        "Diário",
        "Gratidão Especial",
        "Cultos Especiais",
    ...
    public FrmGratidao() {
        setTitle("Cadastro – Donativos de Gratidão");
        setSize(620, 500);
    ...
    private JPanel formulario() {
        ...
        p.add(label("Membro:"));
        cbMembro = new JComboBox<>(); p.add(cbMembro);

        p.add(label("Valor (R$):"));
        txtValor = campo(true); p.add(txtValor);

        p.add(label("Tipo de Gratidão:"));
        cbTipo = new JComboBox<>(TIPOS_GRATIDAO);
        p.add(cbTipo);

        p.add(label("Data (dd/MM/yyyy HH:mm):"));
    ...
    private void salvar() {
        try {
            ...
            g.setCodMembro(m.getCodMembro());

            g.setValor(Double.parseDouble(txtValor.getText().replace(",", ".")));
            g.setTipo(cbTipo.getSelectedItem().toString());
            g.setData(LocalDateTime.parse(txtData.getText(), dtf));

            dao.inserir(g);
            carregarLista();
            JOptionPane.showMessageDialog(this,"Gratidão registrada!");
            novo();

        } catch (Exception e) {
            JOptionPane.showMessageDialog(
                    this,
                    "Erro ao salvar.\nVerifique o valor e a data."
            );
        }
    }
    ...
    private void editar() {
        ...
        try {
            Gratidao g = new Gratidao();
            g.setCodGratidao(Integer.parseInt(txtCodigo.getText()));
    ...
    private void mostrar() {
        Gratidao g = lista.get(posicao);

        txtCodigo.setText(String.valueOf(g.getCodGratidao()));
        txtValor.setText(String.format("%.2f", g.getValor()));
        txtData.setText(g.getData().format(dtf));
        cbTipo.setSelectedItem(g.getTipo());

        for (int i = 0; i < cbMembro.getItemCount(); i++) {
            if (cbMembro.getItemAt(i).getCodMembro() == g.getCodMembro()) {
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(FrmGratidao::new);
    }
}
